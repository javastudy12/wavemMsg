...
public class FontTest {
	...
	public static void main(String[] args) throws Exception {
		...
		int imageWidth = imageMargin + nameMaxWidth + imageMargin + textMaxWidth + imageMargin;
		
		BufferedImage image = new BufferedImage(imageWidth, (int) (font.length * fontsize * 2), BufferedImage.TYPE_INT_RGB);
		...
		for (int i = 0; i < font.length; i++) {
			...
			g.drawString(getDispFontName(font[i]), imageMargin, i * 2 * fontsize + fontsize);
			g.setFont(font[i].deriveFont(fontsize));
			g.drawString(text, imageMargin + nameMaxWidth + imageMargin, i * 2 * fontsize + fontsize);
		}
		
		ImageIO.write(image, "png", new File("c:/fonttest.png"));
	}
